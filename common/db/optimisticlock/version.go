...
func (v VersionUpdateClause) ModifyStatement(stmt *gorm.Statement) {
	...
	if reflect.Indirect(dv).Kind() == reflect.Struct {
		if !stmt.Unscoped {
			// before version is val, zero := v.Field.ValueOf(stmt.Context, reflect.ValueOf(stmt.ReflectValue))
			// stmt.ReflectValue is zero value when use gorm gen, and ReflectValue's type is a model struct

			if val, zero := v.Field.ValueOf(stmt.Context, reflect.ValueOf(stmt.Dest)); !zero {
				if version, ok := val.(Version); ok && version.Valid {
